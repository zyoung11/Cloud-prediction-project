...
class XLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias):
        ...
        self.gate_i = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))
        self.gate_f = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))
        self.gate_o = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))
    ...
    def forward(self, input_tensor, cur_state):
        ...
        i = torch.exp(-torch.abs(cc_i * self.gate_i))
        f = torch.exp(-torch.abs(cc_f * self.gate_f))
        o = torch.exp(-torch.abs(cc_o * self.gate_o))
        g = torch.tanh(cc_g)
        ...
        # 计算每个空间位置的协方差
        X_flat = input_tensor.view(batch_size, self.input_dim, -1)
        c_flat = c_cur.view(batch_size, self.hidden_dim, -1)
        
        # 对每个空间位置分别计算均值
        X_mean = X_flat.mean(dim=1, keepdim=True)
        c_mean = c_flat.mean(dim=1, keepdim=True)
        ...
        # 计算每个位置的协方差
        scale = 1.0 / (self.input_dim * self.hidden_dim - 1)
        Cov_t = torch.einsum('bci,bhi->bh', X_centered, c_centered) * scale
        
        # 将协方差扩展到正确的空间维度
        Cov_t = Cov_t.view(batch_size, self.hidden_dim, 1, 1)
        Cov_t = Cov_t.expand(-1, -1, height, width)

        # 状态更新
        c_next = f * c_cur + i * g + self.alpha * Cov_t
...
g_scheduler = torch.optim.lr_scheduler.OneCycleLR(
    g_optimizer,
    max_lr=0.01,  # 最高学习率
    steps_per_epoch=1968,  # 每个epoch的步数
    epochs=100,  # 总训练轮数
    pct_start=0.3,  # 学习率上升所占的比例
    anneal_strategy='cos',  # 使用余弦策略衰减
    final_div_factor=1e4,  # 最终学习率与初始学习率的比率
    verbose=True
)
...
def train_step(input_seq, target_seq):
    ...
    if generator.global_step % gen_updates == 0:
        g_optimizer.step()
        # g_scheduler.step()  # 添加这一行, 学习率调度器(3)
        generator.zero_grad()
