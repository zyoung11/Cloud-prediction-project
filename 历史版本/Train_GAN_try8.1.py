...
# 创建 DataLoader
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=4, shuffle=False) # 设置一批有几个sequence
...
class DiffusionGenerator(nn.Module):
    def __init__(self, input_channels, hidden_channels, num_layers, diffusion_steps=1000, beta_start=0.0001, beta_end=0.02):
    ...
    def forward(self, x):
        ...
        for step in reversed(range(self.diffusion_steps)):
            t = torch.full((x_t.size(0),), step, dtype=torch.long, device=x_t.device)
...
class TransformerGenerator(nn.Module):
    def __init__(self, input_channels, hidden_channels, num_layers, nhead=4, dim_feedforward=256, num_transformer_layers=4):
        ...
        # Transformer组件
        transformer_encoder_layer = TransformerEncoderLayer(d_model=hidden_channels, nhead=nhead, dim_feedforward=dim_feedforward)
        self.transformer_encoder = TransformerEncoder(transformer_encoder_layer, num_layers=num_transformer_layers)

        transformer_decoder_layer = TransformerDecoderLayer(d_model=hidden_channels, nhead=nhead, dim_feedforward=dim_feedforward)
        self.transformer_decoder = TransformerDecoder(transformer_decoder_layer, num_layers=num_transformer_layers)

    def forward(self, x):
        # x形状：(batch, time_steps, channels, height, width)
        batch_size, time_steps, _, height, width = x.size()
        ...
        # 展平并准备好供Transformer使用
        x_t = x_t.view(batch_size, -1, self.convlstm_cells[0].hidden_dim)
        transformer_output = self.transformer_encoder(x_t)

        # Transformer解码器用于预测下一个时间步
        transformer_output = transformer_output.view(batch_size, height, width, self.convlstm_cells[0].hidden_dim).permute(0, 3, 1, 2)
        output = self.separation(transformer_output)
...
if fine_tune:
    ...
    # 加载预训练模型
    generator = TransformerGenerator(input_channels=1, hidden_channels=64, num_layers=4).to(device)
...
def train_step(input_seq, target_seq):
    ...
    total_g_loss = g_loss + 0.1 * adversarial_loss  # 可以根据需要调整对抗损失的权重
