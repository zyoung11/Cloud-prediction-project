...
class ImageToImageDataset(Dataset):
    def __init__(self, input_root_dir, label_root_dir, sequence_length, transform=None):
        ...
        self.image_filenames = sorted([f for f in self.input_root_dir.glob('*.npy') if f.is_file()])
        self.label_filenames = [self.label_root_dir / f.name for f in self.image_filenames]

        print(f"读取到 {len(self.image_filenames)} 张训练图像.")
        print(f"读取到 {len(self.label_filenames)} 张标签图像.\n")
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.sequence_length):
            ...
            input_image = np.load(input_image_path)  # 读取 .npy 文件
            input_image = Image.fromarray(input_image)  # 将 numpy 数组转换为 PIL 图像
        ...
        label_image = np.load(label_image_path)  # 读取 .npy 文件
        label_image = Image.fromarray(label_image)  # 将 numpy 数组转换为 PIL 图像
