...
model_time_str = input("请输入这轮训练的模型预测时间: ") # 获取用户输入
...
for epoch in tqdm(range(num_epochs), desc="Epochs"):
    ...
    for input_seq, target_seq in  tqdm(train_dataloader, desc="运算量", leave=False):
        input_seq, target_seq = input_seq.to(device), target_seq.to(device)
    ...
    print(f"训练轮次[{epoch+1}/{num_epochs}] | "
          ...
          f"相对误差绝对值: {val_rae:.4f} | 均方误差: {val_rmse:.4f} | "
          f"结构相似性: {val_ssim:.4f} | 结构相似性结合平均误差绝对值: {val_ssim_mae:.4f} | "
    ...
    results['训练轮次'].append(epoch_data)
    results['生成器损失'].append(avg_g_loss)
    results['判别器损失'].append(avg_d_loss)
    results['验证集损失'].append(val_loss)
    results['峰值信噪比'].append(val_psnr)
    results['相对误差绝对值'].append(val_rae)
    results['均方误差'].append(val_rmse)
    results['结构相似性'].append(val_ssim)
    results['结构相似性结合平均误差绝对值'].append(val_ssim_mae)
    results['生成器学习率'].append(g_optimizer.param_groups[0]['lr'])
    results['判别器学习率'].append(d_optimizer.param_groups[0]['lr'])
    ...
    if (epoch + 1) % 5 == 0 or epoch == num_epochs - 1:
        ...
        file_path = log_dir / f'{model_time_str}_training_data_epochs_{epoch+1}.csv'
    ...
    if (epoch + 1) % 5 == 0 or epoch == num_epochs - 1:
        # 保存生成器模型
        gen_model_path = gen_model_dir / f"{model_time_str}_生成器_模型_epoch_{epoch+1}.pth"
        print(f"\n生成器模型保存到: {gen_model_path}")
        torch.save(generator.state_dict(), gen_model_path)
        
        # 保存判别器模型
        disc_model_path = disc_model_dir / f"{model_time_str}_判别器_模型_epoch_{epoch+1}.pth"
