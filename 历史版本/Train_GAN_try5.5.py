...
class ImageToImageDataset(Dataset):
    def __init__(self, root_dir, sequence_length=6, transform=None):
        self.root_dir = Path(root_dir)
        self.transform = transform
        self.sequence_length = sequence_length

        self.samples = []
        input_files = sorted(self.root_dir.glob('*.npy'))
        print(f"在 {root_dir} 中找到 {len(input_files)} 个 .npy 文件")

        for i in range(len(input_files) - sequence_length - 3):  # -3 是因为我们需要3个额外的标签
            start_time = datetime.strptime(input_files[i].stem, "%Y%m%d%H%M%S")
            if all((datetime.strptime(f.stem, "%Y%m%d%H%M%S") - start_time).total_seconds() / 600 == j 
                   for j, f in enumerate(input_files[i:i+sequence_length+18])):  # +18 for 3 hours of labels
                self.samples.append(input_files[i].stem)

        print(f"创建了 {len(self.samples)} 个有效样本")
        if not self.samples:
            raise ValueError("没有找到有效的样本序列")

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        start_time = datetime.strptime(self.samples[idx], "%Y%m%d%H%M%S")
        
        input_sequence = []
        for i in range(self.sequence_length):
            input_time = start_time + timedelta(minutes=10*i)
            input_file = self.root_dir / f'{input_time.strftime("%Y%m%d%H%M%S")}.npy'
        ...
        label_images = []
        for i in range(3):  # 3个目标时间点：1小时后，2小时后，3小时后
            label_time = start_time + timedelta(hours=i+1)
            label_file = self.root_dir / f'{label_time.strftime("%Y%m%d%H%M%S")}.npy'
            if not label_file.exists():
                raise FileNotFoundError(f"标签文件 {label_file} 不存在")
            label_image = np.load(label_file)
            label_image = self.normalize(label_image)
            if self.transform:
        ...
        return input_sequence, label_images

    def normalize(self, image):
        # 将数据从 [175, 340] 范围映射到 [0, 1] 范围
        min_val = 175
        max_val = 340
...
'''
# 创建训练和测试数据集
train_dataset = ImageToImageDataset(root_dir=Path("data_c"),
                                    sequence_length=6,
                                    transform=transform)

test_dataset = ImageToImageDataset(root_dir=Path("data_c"),
                                   sequence_length=6,
                                   transform=transform)
'''
# 创建训练和测试数据集
train_dataset = ImageToImageDataset(root_dir=Path("data_c"), sequence_length=6, transform=transform)
test_dataset = ImageToImageDataset(root_dir=Path("data_c"), sequence_length=6, transform=transform)
...
if len(train_dataset) == 0 or len(test_dataset) == 0:
    raise ValueError("数据集为空，请检查数据路径和文件是否正确。")

# 测试数据集的第一个样本
first_input, first_label = train_dataset[0]
print(f"第一个输入样本的形状: {first_input.shape}")
print(f"第一个标签样本的形状: {first_label.shape}")

# 测试数据集的最后一个样本
last_input, last_label = train_dataset[-1]
print(f"最后一个输入样本的形状: {last_input.shape}")
print(f"最后一个标签样本的形状: {last_label.shape}")
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        if x_t is None:
            x_t = self.fusion(x[:, time_steps - 3])

        # 解码器
        outputs = []
        for _ in range(3):  # 预测接下来的三个小时
            for l in range(self.num_layers):
                h[l], c[l] = self.convlstm_cells[l](x_t, (h[l], c[l]))
                x_t = h[l]
        ...
        for input_seq, target_seq in val_loader:
            ...
            for t in range(3):
                for i in range(gen_output.size(0)):
                    output_img = gen_output[i, t].cpu().numpy().squeeze()
                    target_img = target_seq[i, t].cpu().numpy().squeeze()
            ...
            num_samples += gen_output.size(0)
    
    avg_val_loss = total_val_loss / len(val_loader)
    avg_psnr = [total / num_samples for total in total_psnr]
    avg_rae = [total / num_samples for total in total_rae]
    avg_rmse = [total / num_samples for total in total_rmse]
    
    return avg_val_loss, avg_psnr, avg_rae, avg_rmse
...
for epoch in tqdm(range(num_epochs), desc="Epochs"):
    ...
    for input_seq, target_seq in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{num_epochs}", leave=False):
        input_seq, target_seq = input_seq.to(device), target_seq.to(device)
    ...
    val_loss, val_psnr, val_rae, val_rmse = validate(test_dataloader, generator, device, g_loss_fn)

    print(f"  训练轮次[{epoch+1}/{num_epochs}] | "
        f"生成器损失: {avg_g_loss:.4f} | 判别器损失: {avg_d_loss:.4f} | "
        f"验证集损失: {val_loss:.4f} | "
        f"峰值信噪比: {val_psnr[0]:.4f}/{val_psnr[1]:.4f}/{val_psnr[2]:.4f} dB | "
        f"相对误差绝对值: {val_rae[0]:.4f}/{val_rae[1]:.4f}/{val_rae[2]:.4f} | "
        f"均方根误差: {val_rmse[0]:.4f}/{val_rmse[1]:.4f}/{val_rmse[2]:.4f} |"
...
batch_size=4,sequence_length=4 时,每个批次会包含4个图像序列:
第一个批次:
序列1:1.jpg, 2.jpg, 3.jpg, 4.jpg
序列2:2.jpg, 3.jpg, 4.jpg, 5.jpg
序列3:3.jpg, 4.jpg, 5.jpg, 6.jpg
序列4:4.jpg, 5.jpg, 6.jpg, 7.jpg
第二个批次:
序列1:5.jpg, 6.jpg, 7.jpg, 8.jpg
序列2:6.jpg, 7.jpg, 8.jpg, 9.jpg
序列3:7.jpg, 8.jpg, 9.jpg, 10.jpg
序列4:8.jpg, 9.jpg, 10.jpg, 11.jpg
