    ...
    with torch.no_grad():
        for input_seq, target_seq in val_loader:
            ...
            for i in range(gen_output.size(0)):
                output_img = gen_output[i].cpu().numpy().squeeze()
...
for epoch in tqdm(range(num_epochs), desc="Epochs"):
    ...
    if (epoch + 1) % 5 == 0 or epoch == 0:
        # 保存生成器模型
        gen_model_path = MODEL_PATH / f"小时_生成器_模型_epoch_{epoch+1}.pth"
        print(f"\n生成器模型保存到: {gen_model_path}")
        torch.save(generator.state_dict(), gen_model_path)
        
        # 保存判别器模型
        disc_model_path = MODEL_PATH / f"小时_判别器_模型_epoch_{epoch+1}.pth"
