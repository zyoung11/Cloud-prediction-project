...
class ChannelAttention(nn.Module):
    def __init__(self, in_channels, reduction=16):
        ...
        # 确保通道数不会被减少到 0
        reduced_channels = max(in_channels // reduction, 1)  # 确保至少为 1
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([avg_out, max_out], dim=1)  # 拼接后通道数为2
        x = self.conv1(x)
...
class XLSTMCellWithCBAM(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias):
        super(XLSTMCellWithCBAM, self).__init__()
        ...
        # CBAM 注意力模块
        self.cbam = CBAM(4 * hidden_dim)  # CBAM模块，通道数与卷积输出通道数一致

        # 可学习的门控机制
        self.gate_i = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 输入门缩放参数
        self.gate_f = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 遗忘门缩放参数
        self.gate_o = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 输出门缩放参数

        # 窥视孔连接的权重
        self.peep_i = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 输入门窥视孔权重
        self.peep_f = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 遗忘门窥视孔权重
        self.peep_o = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 输出门窥视孔权重
    ...
    def forward(self, input_tensor, cur_state):
        ...
        # 经过CBAM模块
        combined_conv = self.cbam(combined_conv)

        # 划分卷积的输出为 i, f, o, g
        cc_i, cc_f, cc_o, cc_g = torch.split(combined_conv, self.hidden_dim, dim=1)

        # 使用可学习的门控机制，加入窥视孔连接
        i = torch.sigmoid(cc_i * self.gate_i + c_cur * self.peep_i)  # 输入门加入窥视孔
        f = torch.sigmoid(cc_f * self.gate_f + c_cur * self.peep_f)  # 遗忘门加入窥视孔
        o = torch.sigmoid(cc_o * self.gate_o + c_cur * self.peep_o)  # 输出门加入窥视孔
        g = torch.tanh(cc_g)
...
class Generator(nn.Module):
    def __init__(self, input_channels, hidden_channels, num_layers):
        ...
        self.convlstm_cells = nn.ModuleList([XLSTMCellWithCBAM(hidden_channels, hidden_channels, (3, 3), True) for _ in range(num_layers)])
