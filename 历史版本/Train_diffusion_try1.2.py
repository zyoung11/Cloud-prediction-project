...
class DiffusionModel(nn.Module):
    def __init__(self, input_channels, timesteps=100):
        super(DiffusionModel, self).__init__()
        self.timesteps = timesteps
        self.noise_schedule = torch.linspace(1e-4, 0.02, timesteps)
    ...
    def forward(self, x):
        ...
        noisy_x = x + noise * self.noise_schedule[t].view(-1, 1, 1, 1).to(x.device)

        # 去噪过程
        x = self.leaky_relu(self.conv1(noisy_x))
        x = self.leaky_relu(self.conv2(x))
        x = self.leaky_relu(self.conv3(x))
        denoised_x = self.conv4(x)
...
class Generator(nn.Module):
    def __init__(self, input_channels, hidden_channels, num_layers):
        ...
        self.diffusion_model = DiffusionModel(hidden_channels)
    ...
    def forward(self, x):
        ...
        # 扩散模型的反向过程，从加入噪声开始逐步去噪
        timesteps = 100
        for t in reversed(range(timesteps)):
            x_t = self.diffusion_model(x_t, t)
