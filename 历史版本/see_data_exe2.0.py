...
class MainWindow(QMainWindow):
    ...
    def load_and_plot_data(self):
        ...
        if file_path:
            ...
            data.columns = ['avg_g_loss', 'avg_d_loss', 'val_loss', 'val_psnr', 'val_rae', 'val_rmse', 'val_ssim', 'val_ssim_mae', 'val_real_score', 'val_fake_score', 'g_lr', 'd_lr']

            self.canvas.figure.clear()
            axs = [self.canvas.figure.add_subplot(2, 4, i + 1) for i in range(7)]  # 修改为2行4列，显示7个图表
            ...
            # 绘制真实与假冒分数
            axs[5].plot(data.index, data['val_real_score'], label='Real Score')
            axs[5].plot(data.index, data['val_fake_score'], label='Fake Score')
            axs[5].set_title('Real and Fake Score')
            axs[5].set_xlabel('Epoch')
            axs[5].set_ylabel('Score')
            axs[5].legend()

            # 绘制学习率
            axs[6].plot(data.index, data['g_lr'], label='Generator Learning Rate')
            axs[6].plot(data.index, data['d_lr'], label='Discriminator Learning Rate')
            axs[6].set_title('Learning Rates')
