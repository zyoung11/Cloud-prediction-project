...
class XLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias):
        ...
        self.conv = nn.Conv2d(in_channels=self.input_dim + self.hidden_dim,
                              out_channels=7 * self.hidden_dim,  # 注意这里变成了7
    ...
    def forward(self, input_tensor, cur_state):
        ...
        cc_i, cc_f, cc_g, cc_o, cc_i_prime, cc_f_prime, cc_g_prime = torch.split(combined_conv, self.hidden_dim, dim=1)
        ...
        c_next = f * c_cur + i * g + f_prime * c_cur + i_prime * g_prime
...
class Generator(nn.Module):
    def __init__(self, input_channels, hidden_channels, num_layers):
        ...
        self.xlstm_cells = nn.ModuleList([XLSTMCell(hidden_channels, hidden_channels, (3, 3), True) for _ in range(num_layers)])
        self.separation = SeparationModule(hidden_channels, input_channels)

    def forward(self, x):
        batch_size, time_steps, _, height, width = x.size()
        
        h = [torch.zeros(batch_size, self.xlstm_cells[0].hidden_dim, height, width).to(x.device) for _ in range(self.num_layers)]
        c = [torch.zeros(batch_size, self.xlstm_cells[0].hidden_dim, height, width).to(x.device) for _ in range(self.num_layers)]

        for t in range(time_steps):
            x_t = self.fusion(x[:, t])
            for l in range(self.num_layers):
                h[l], c[l] = self.xlstm_cells[l](x_t, (h[l], c[l]))
                x_t = h[l]
