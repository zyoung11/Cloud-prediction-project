...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QPushButton, QVBoxLayout, QHBoxLayout, QWidget,
                             QFileDialog, QLabel, QTextEdit, QSizePolicy, QSpacerItem)
...
def process_single_npy_file(params):
    npy_path, target_dir, log_queue = params
    ...
    try:
        ...
        if npy_data.ndim == 2:
            plt.imshow(npy_data, cmap='gray')
            # log_queue.put(f"灰度图像: {filename}")
        elif npy_data.ndim == 3:
            if npy_data.shape[2] == 3:
                plt.imshow(npy_data)
                log_queue.put(f"RGB图像: {filename}")
            elif npy_data.shape[2] == 4:
                plt.imshow(npy_data)
                log_queue.put(f"RGBA图像: {filename}")
            else:
                log_queue.put(f"Error: {filename} has an unsupported channel size ({npy_data.shape[2]}). Skipping...")
                return
        else:
            log_queue.put(f"Error: {filename} has an unsupported number of dimensions ({npy_data.ndim}). Skipping...")
            return
        ...
        log_queue.put(f"Converted {filename} to {png_filename}")
    
    except Exception as e:
        log_queue.put(f"Error processing file: {str(e)}")
        gc.collect()  # 在发生异常时也尝试释放内存


def npy_to_png_data(source_dir, num_workers=4, log_callback=None):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    target_dir = os.path.join(current_dir, 'converted_images_temp')
    if not os.path.exists(target_dir):
        os.makedirs(target_dir)

    npy_files = [os.path.join(source_dir, filename) for filename in os.listdir(source_dir) if filename.endswith('.npy')]

    with Manager() as manager:
        log_queue = manager.Queue()
        params = [(npy_file, target_dir, log_queue) for npy_file in npy_files]

        def log_worker():
            while True:
                try:
                    message = log_queue.get(timeout=3)  # 使用超时避免死锁
                    if message == "DONE":
                        break
                    if log_callback:
        ...
        log_thread = Thread(target=log_worker)
        log_thread.start()

        try:
            with Pool(num_workers) as pool:
                for i, _ in enumerate(pool.imap_unordered(process_single_npy_file, params), 1):
                    gc.collect()  # 每处理完一个文件，强制执行垃圾回收
        except Exception as e:
            log_queue.put(f"Error in pool processing: {str(e)}")
        finally:
            log_queue.put("DONE")
            log_thread.join()
...
class ModernGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.display_mode = 'side_by_side'  # 初始模式为左右对比模式
        self.initUI()
        self.load_models()

    def initUI(self):
        self.setWindowTitle('AI云图预测 - 样本模式')
        self.setWindowIcon(QIcon(load_image('icon1.ico')))
        #self.setGeometry(100, 100, 800, 800)
        # 如果有保存的窗口状态，恢复窗口的大小和位置
        if DataModeGUI.last_geometry:
            self.setGeometry(DataModeGUI.last_geometry)
        ...
        self.model_buttons = ["10min", "30min", "1h", "2h", "3h"]
    ...
    def select_input_folder(self):
        ...
        if self.input_folder:
            ...
            self.prev_btn.setEnabled(False)  # 确保导航按钮保持不可点击状态
            self.next_btn.setEnabled(False)
            self.toggle_mode_btn.setEnabled(False)
    ...
    def toggle_display_mode(self):
        if self.display_mode == 'side_by_side':
            self.display_mode = 'animation'  # 切换到动图模式
            self.toggle_mode_btn.setText("切换对比模式")
            self.run_inference_gif()  # 切换到动图模式后运行新的推理方法
        else:
            self.display_mode = 'side_by_side'  # 切换到左右对比模式
            self.toggle_mode_btn.setText("切换动画模式")
            self.run_inference()  # 切换回对比模式后运行默认的推理方法
        ...
        for file in sorted(os.listdir(self.input_folder)):
            if file.endswith(('.png', '.jpg', '.jpeg')):
                ...
                if converted_images_folder is None:
                    ...
                    self.log("数据预处理中...")
                    QApplication.processEvents()
                    npy_to_png_data(self.input_folder, num_workers=cpu_count(), log_callback=self.log)    
                    converted_images_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'converted_images_temp')
                # 将转换后的 .png 文件添加到输入图像列表中
                png_file = file.replace('.npy', '.png')
                png_path = os.path.join(converted_images_folder, png_file)
