...
class XLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias):
        ...
        # 添加窥视孔权重
        self.peep_i = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 输入门窥视孔
        self.peep_f = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 遗忘门窥视孔
        self.peep_o = nn.Parameter(torch.ones(1, hidden_dim, 1, 1))  # 输出门窥视孔
    ...
    def forward(self, input_tensor, cur_state):
        ...
        # 使用可学习的门控机制和窥视孔
        i = torch.sigmoid(cc_i * self.gate_i + c_cur * self.peep_i)  # 加入c_cur的窥视孔
        f = torch.sigmoid(cc_f * self.gate_f + c_cur * self.peep_f)  # 加入c_cur的窥视孔
        o = torch.sigmoid(cc_o * self.gate_o + c_cur * self.peep_o)  # 加入c_cur的窥视孔
        g = torch.tanh(cc_g)
