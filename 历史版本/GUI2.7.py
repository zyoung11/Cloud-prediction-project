...
class HoverButton(QPushButton):
    ...
    def enterEvent(self, event):
        self.setStyleSheet(self.default_style + self.hover_style + "border-radius: 20px;")
        super().enterEvent(event)
...
class InferenceGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        self.input_btn = HoverButton("Select Input Folder")
        ...
        self.output_btn = HoverButton("Select Output Folder")
        ...
        self.start_btn = HoverButton("Start Inference")
        ...
        self.toggle_mode_btn = HoverButton("Toggle Mode")
    ...
    def run_inference(self):
        ...
        if self.display_mode == 'overlay':
            ...
            self.display_combined_images(current_input[-1], overlay_image_path)
        else:
            # 显示原始对比图像和生成图像
    ...
    def display_combined_images(self, original_image_path, generated_image_path):
        # 加载原图和生成图
        original_pixmap = QPixmap(original_image_path)
        generated_pixmap = QPixmap(generated_image_path)

        # 获取标签的大小，确保图像按比例缩放并居中显示
        label_width = self.input_image_label.width()
        label_height = self.input_image_label.height()

        # 按比例缩放图像，使它们适应标签的大小
        original_pixmap = original_pixmap.scaled(label_width, label_height, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        generated_pixmap = generated_pixmap.scaled(label_width, label_height, Qt.KeepAspectRatio, Qt.SmoothTransformation)

        # 将原图和生成图叠加，生成图放在原图上方
        combined_pixmap = QPixmap(original_pixmap.size())
        combined_pixmap.fill(Qt.transparent)  # 透明底层
        painter = QPainter(combined_pixmap)
        painter.drawPixmap(0, 0, original_pixmap)
        painter.drawPixmap(0, 0, generated_pixmap)
        painter.end()

        # 在两个标签中显示组合图像
        self.input_image_label.setPixmap(combined_pixmap)
        self.output_image_label.setPixmap(combined_pixmap)
    ...
    def create_overlay_image(self, original_image_path, generated_image_path):
        ...
        generated_array[mask, 3] = 128  # 降低透明度
