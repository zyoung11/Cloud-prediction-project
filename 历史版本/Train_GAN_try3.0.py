...
class ImageToImageDataset(Dataset):
    def __init__(self, input_root_dir, label_root_dir, sequence_length=4, transform=None):
        ...
        # 打印读取到的文件数量
        print(f"Found {len(self.image_filenames)} input images.")
        print(f"Found {len(self.label_filenames)} label images.")
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        # Check if time_steps is sufficient for the loop
        if time_steps < 3:
            raise ValueError(f"time_steps should be at least 3, but got {time_steps}")

        # Encoder
        x_t = None  # Initialize x_t to avoid UnboundLocalError
...
class GeneratorLoss(nn.Module):
    ...
    def forward(self, pred, target):
        # 仅使用 pred 的最后一个时间步与 target 进行比较
        pred_last = pred[:, -1, :, :, :]  # 取最后一个时间步
...
def train_step(input_seq, target_seq):
    ...
    # 将 target_seq 扩展一个维度，以匹配 input_seq 的维度
    target_seq_expanded = target_seq.unsqueeze(1)
    
    # 合并时间维度和批次维度
    real_input = torch.cat([input_seq[:, -3:], target_seq_expanded], dim=1)
    real_input = real_input.view(-1, *real_input.shape[2:])  # 合并时间步和批次维度
...
def validate(val_loader):
    ...
    with torch.no_grad():
        for input_seq, target_seq in val_loader:
            ...
            for i in range(gen_output.size(0)):
                # 取生成的最后一个时间步的输出
                output_img = gen_output[i, -1, :, :, :].cpu().numpy().squeeze()  # 只保留空间维度
                target_img = target_seq[i].cpu().numpy().squeeze()

                # 确保转置前的张量是 [height, width, channels] 形状
                if output_img.ndim == 2:  # 如果是单通道灰度图像
                    output_img = output_img[..., np.newaxis]  # 添加一个 channel 维度
    ...
    # print(f"Validation PSNR: {avg_psnr:.4f} dB, Avg Val Loss: {avg_val_loss:.4f}")
...
for epoch in range(num_epochs):
    ...
    val_loss, val_psnr = validate(test_dataloader)
    
    print(f"Epoch [{epoch+1}/{num_epochs}], G Loss: {avg_g_loss:.4f}, D Loss: {avg_d_loss:.4f}, Val Loss: {val_loss:.4f}, Val PSNR: {val_psnr:.4f} dB")
    ...
    with torch.no_grad():
        for i, (X, y) in enumerate(test_dataloader):
            ...
            preds = generator(X)  # preds 现在包含所有时间步的预测结果

            # 保存每个批次中每个时间步的预测结果
            for j in range(preds.size(0)):  # 遍历批次中的每个样本
                for t in range(preds.size(1)):  # 遍历时间步
                    save_image(preds[j, t], f'y_pred/batch_{i+1}_timestep_{t+1}_sample_{j+1}.jpg')
