...
def preprocess_data(src_folder, dest_folder):
    # 创建目标文件夹
    os.makedirs(dest_folder, exist_ok=True)
    
    # 获取所有子文件夹中的 .npy 文件总数
    total_files = sum(len(files) for _, _, files in os.walk(src_folder) if any(f.endswith('.npy') for f in files))
    
    # 使用 tqdm 为文件复制过程添加进度条
...
preprocess_data(src_folder, processed_folder)
